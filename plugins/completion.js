...
module.exports = function (req, res, next) {
  ...
  tabtab.complete("gr", function (err, data) {
    ...
    */
    ...
    if (
      line.substr(0, "gr tag add".length) == "gr tag add" ||
      ...
      switch (data.prev) {
        ...
        default:
          if (
            data.lastPartial &&
            (data.lastPartial.charAt(0) == "#" ||
              data.lastPartial.charAt(0) == "@")
          ) {
            tabtab.log(hashtags, data);
          } else if (
            data.prev &&
            (data.prev.charAt(0) == "#" || data.prev.charAt(0) == "@")
          ) {
            tabtab.log(actions, data);
          } else {
            if (/^-\w?/.test(data.last)) {
              return tabtab.log(
                ["t", "-json", "-", "-help", "-version", "v", "h"],
                data,
                "-"
              );
            }
            tabtab.log(
              ["tag", "list", "status", "config", "help", "version"].concat(
                hashtags
              ),
              data
            );

...
function _getTargets(argv) {
  ...
  if (argv.length > 1) {
    // tag add foo ~/path
    targets = argv.slice(1).map(function (item) {
...
function add(req, res, next) {
  ...
  targets.forEach(function (target) {
    ...
    if (req.format == "human") {
      ...
      console.log(
        JSON.stringify({ op: "add", tag: req.argv[0], path: target })
      );
    }
  });
...
function discover(req, res, next) {
  ...
  while (currentPaths.length > 0 && depth < maxDepth) {
    ...
    try {
      ...
      var hasGit = paths.filter(function (p) {
        return p === ".git";
      });
      gitPaths = gitPaths.concat(
        hasGit.map(function (name) {
  ...
  gitPaths.sort().forEach(function (dir) {
    ...
    humanDir = dir
      .replace(new RegExp("^" + req.gr.homePath + "/"), "~/")
      .replace(/ /g, "\\ ");
...
function applyTags(req, lines) {
  ...
  lines.forEach(function (line) {
    ...
    var parts = splitBySpace(line),
      ...
      confTags =
        req && req.config && req.config.items && req.config.items.tags
          ? req.config.items.tags
          : [];
    ...
    allTags.forEach(function (tag) {
      var shouldHaveTag = tags.indexOf(tag) > -1,
        hasTag =
          Array.isArray(confTags[tag]) && confTags[tag].indexOf(dirname) > -1;

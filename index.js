  ...
  function next(err) {
    ...
    if (!layer) {
      ...
      if (self.directories.length === 0 && self.format == "human") {
        log.warn(
          "No target paths matched. Perhaps no paths are associated with the tag you used?"
        );
      }
      return;
    }
...
Gr.prototype.getTagsByPath = function (cwd) {
  ...
  Object.keys(this.config.items.tags).forEach(function (tag) {
    if (
      Array.isArray(self.config.items.tags[tag]) &&
...
Gr.prototype.dirExist = function () {
  this.directories = this.directories.filter(Boolean).filter(function (path) {
